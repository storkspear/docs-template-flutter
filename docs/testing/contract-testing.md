...
계약 테스트가 **의도된 변경인지 리뷰하도록 강제해요**. 수정 시 테스트가 깨져서 "이거 정말 맞나?" 한 번 더 확인하게 돼요.
...
계약 테스트는 **Critical**. 깨지면 다른 Kit 이 영향받아요. CI (`ci.yml`) 는 `flutter test` 전체 실행에 포함하고, 로컬에서 계약 테스트만 돌리려면:

```bash
flutter test test/kits
```

파일명이 `<kit_name>_contract_test.dart` 라 `**/_contract_test.dart` 같은 글롭은 아무것도 매칭하지 않아요 — 디렉토리 단위로 실행하세요.
