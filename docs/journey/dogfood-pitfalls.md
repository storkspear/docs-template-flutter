...
파생 레포 셋업 · 개발 · 배포 과정에서 **반복적으로 발생하는 함정** 과 해결법을 모아둔 문서예요. 각 항목은 **증상 · 원인 · 해결** 3단계로 정리했어요.

> 여기 없으면 [`FAQ`](./dogfood-faq.md) 나 관련 ADR · 컨벤션 문서를 검색해 보세요.
...
**증상**: 네트워크 · SSL 에러 · 버전 불일치로 실패해요.

**원인**: VPN 또는 방화벽이 pub.dev 를 차단했거나, `pubspec.lock` 이 다른 Flutter 버전에서 생성됐거나, Flutter 버전이 너무 낮은 경우예요.
...
**증상**: `CocoaPods could not find compatible versions` 같은 에러가 나요.

**원인**: Xcode 버전이 낮거나, CocoaPods repo 가 오래됐거나, M1/M2 Mac 의 Rosetta 이슈일 수 있어요.
...
**증상**: `package:<old_name>/...` import 가 남아 있어요.

**원인**: 스크립트가 모든 파일을 치환하지 못해서예요 (dart 코드 · 생성 파일 등).
...
**증상**: `<repo> start -d <android-emulator>` 후 빌드/install 까지 성공하고 앱 프로세스도 살아 있어요. 하지만 에뮬레이터 화면이 까맣고, logcat 에 `D/FlutterRenderer: Width is zero. 0,0` 이 반복돼요.

**원인**: AVD 의 `hw.gpu.mode=auto` 가 M1/M2/M3 Mac 에서 잘못된 GPU 백엔드(software fallback) 를 선택해 Flutter surface 가 0×0 으로 잡혀요.
...
**대안**: Android Studio → Device Manager → AVD 편집 → "Graphics" 를 명시적으로 "Hardware - GLES 2.0" 으로 선택해요. 실 Android 기기에선 발생하지 않아요.

---

### ❌ Android 빌드 후 앱 launch 시 `ClassCastException: MainActivity cannot be cast to FlutterFragmentActivity`

**증상**: APK install 은 성공하는데 앱이 시작 직후 강제종료돼요. logcat 의 `GeneratedPluginRegistrant` 에 위 예외가 찍혀요.

**원인**: 일부 Flutter 플러그인 (`flutter_naver_login` 등) 이 MainActivity 를 `FlutterFragmentActivity` 로 캐스팅해요. 기본 `FlutterActivity` 만 상속하면 이 캐스트에 실패해요.

**해결**: `android/app/src/main/kotlin/.../MainActivity.kt` 에서 `FlutterFragmentActivity` 로 변경해요 (템플릿엔 이미 적용돼 있어요).
```kotlin
...
**원인**: `google-services.json` 안에 등록된 package_name 이 dev flavor 의 `applicationId.dev` 와 매칭되지 않아서예요. 보통 `<repo> dev init` 이 끝나면 자동으로 해결되지만, prod 의 google-services.json 을 dev 자리에 복사한 경우에 발생해요.
...
**증상**: 빌드는 성공하는데, 앱 launch 후 "구글로 로그인" 을 탭하면 `signInWithOptions:` 호출 직후 크래시가 나요. Info.plist 의 `GIDClientID` 가 `(GID_CLIENT_ID)` 그대로거나 비어 있어요.

**원인**: `AppEnv-secrets-<env>.xcconfig` (gitignored, `AppEnv-<env>.xcconfig` 가 optional include) 가 없거나 `GID_CLIENT_ID` 가 빈 채로 빌드된 거예요. `<env> link-oauth` 실행이 누락됐거나, GoogleService-Info-<env>.plist 가 없거나, plist 에 CLIENT_ID 키가 빠진 경우 (Firebase Auth Google 사용 설정을 안 한 경우) 이렇게 돼요.
...
**원인**: firebase CLI 는 project 생성 (`projects:create`) 은 지원하지만 **삭제는 지원하지 않아요** (의도된 한계예요). Firebase project 는 내부적으로 GCP project 라서, 삭제엔 GCP 측 도구가 필요해요.
...
**원인**: 기본 `gh auth login` 시 발급되는 토큰엔 `delete_repo` scope 가 빠져 있어요. 안전을 위해 default 는 read/write 만 포함해요.
...
**증상**: 빌드는 되는데 홈 화면 대신 스플래시에 멈춰요.

**원인**: `AppKits.attachContainer(container)` 호출이 누락됐거나, `install` 리스트에 필수 Kit 이 빠진 경우예요.

**해결**: `lib/main.dart` 의 순서를 확인해요.
...
**증상**: `AppKits.install` 시 StateError 가 나요.

**원인**: `auth_kit` 만 활성화하고 `backend_api_kit` 을 뺀 경우예요.

**해결**: `app_kits.yaml` · `main.dart` 둘 다에 `backend_api_kit` 을 추가해요.
...
**증상**: 빌드는 되지만 validator 가 이슈를 지적해요.
...
**증상**: 크래시 · 이벤트가 대시보드에 안 보여요.

**원인**: DSN · API Key 가 주입되지 않아 Debug 폴백으로 동작 중이거나, Release 빌드가 아니거나 (Debug 모드), PII 차단 필터에 걸린 경우예요.
...
**증상**: `S.of(context).newKey` 에 빨간 줄이 떠요.

**원인**: ARB 에 키를 추가한 뒤 `flutter gen-l10n` 을 안 돌린 경우예요.
...
**추가 체크**: `app_ko.arb` · `app_en.arb` 둘 다에 키가 있어야 해요. 한쪽만 있으면 gen-l10n 이 실패해요.

---

### ❌ ViewModel 에서 `BuildContext` 쓰려고 함

**증상**: 스낵바 · 다이얼로그를 ViewModel 에서 띄우고 싶어져요.

**원인**: 디자인 위반이에요. ViewModel 은 UI 에 의존하지 않아요.

**해결**: State 에 플래그를 세팅하고 Screen 이 반응하게 해요.
...
**원인**: `schemaVersion` 은 올렸는데 `onUpgrade` 에 해당 마이그레이션을 안 작성한 경우예요.
...
`RefreshIndicator` 는 `Future` 가 완료될 때까지 스피너를 유지해요.
...
**증상**: AAB 업로드 시 서명 불일치 에러가 나요.

**원인**: Play App Signing 의 upload 인증서와 keystore 가 달라서예요.
...
**원인**: `pubspec.yaml` 의 `+N` build number 가 Play 에 이미 올라간 값과 같거나 작아서예요.

**해결**: `version: 1.2.3+46` 처럼 `+` 뒤 숫자를 올려요.

---

### ❌ TestFlight 앱 "Invalid Binary"

**증상**: Upload 후 "Invalid Binary" 이메일이 와요.

**원인**: 심볼 파일 누락 (bitcode), 필수 Capabilities 미추가, Bundle ID 불일치가 대표적이에요.

**해결**: App Store Connect 의 Activity 에서 실패 사유를 확인해요. 대부분은 이걸로 잡혀요:
```bash
...
**증상**: Actions 는 초록불인데 Play Console 에 없어요.

**원인**: Service account 권한 부족 (Android Publisher), JSON key 만료·오류, Track 이름 오타 (`internal` vs `production`) 중 하나인 경우가 대부분이에요.
...
**증상**: 프로덕션 크래시 원인을 추적할 수 없어요.

**원인**: `--obfuscate` 는 했는데 심볼 업로드를 안 한 거예요.
...
**증상**: `flutter build apk --release` 또는 스토어 빌드 직후 앱이 즉시 죽어요. 로그에 다음 메시지가 나와요:
```text
...
**해결**: `ref.read` 만 사용해요 (단발성 조회). watch 는 ViewModel 을 watch 하는 위젯에서만 써요.
...
막혔는데 해결이 안 되면 GitHub Issue 에 등록해 주세요 — 같은 함정이 다음 파생 레포에선 이 문서에 추가돼 있도록요.
