...
- **`_dio.fetch` 재귀 가능성**: 재시도한 요청이 또 401 이면? Dio 에 "같은 요청은 다시 인터셉트하지 않는" 내부 플래그는 **없어요** — 재시도 요청도 전체 인터셉터 체인을 다시 타므로, 또 401 이면 `onError` 가 재진입해 refresh 를 다시 시도할 수 있어요. 서버가 새 토큰에도 계속 401 을 주는 병리적 케이스에선 반복 가능성이 남아요 — 염두.
