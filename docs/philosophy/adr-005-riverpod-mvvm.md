...
**Status**: Accepted. 현재 유효. 2026-04-24 작성 / 2026-07-15 수치 재실측. `flutter_riverpod ^2.6.1` 사용, `lib/common/providers.dart` (148줄) 중앙 DI, ViewModel 3개 (`login` / `password_reset` / `verify_email`) 파일럿.
...
- **글로벌 provider 파일 (`providers.dart`) 이 비대해질 위험**: 현재 148줄. 30개 넘으면 쪼개야 해요. 아직은 유지.
...
**전역 DI**
- [`lib/common/providers.dart`](https://github.com/storkspear/template-flutter/blob/main/lib/common/providers.dart) — 148줄, 모든 전역 Provider 정의
