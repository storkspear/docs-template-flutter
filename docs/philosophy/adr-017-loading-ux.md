...
로딩 상황을 **4가지 UX 패턴** 으로 규약화했어요 — **Skeleton** (첫 진입) · **Pull-to-refresh** (새로고침) · **버튼 스피너** (버튼 액션 — 진행 중 비활성) · **TopProgressBar** (백그라운드 작업). **풀스크린 블로킹 스피너 금지**. 각 패턴이 다른 맥락에 쓰여서, 개발자가 "지금 어떤 로딩?" 만 판단하면 자연스럽게 통일된 UX 가 나와요.
...
| **버튼 액션** (로그인 · 저장) | 버튼 스피너 (액션 중 비활성) | `PrimaryButton(isLoading: true)` | 동작 중임을 버튼에 국한 |
...
PrimaryButton(
  ...
  isLoading: state.isLoading,       // ← true 면 내부 스피너 + 버튼 비활성
  onPressed: () => vm.save(),
)
```

`PrimaryButton` 위젯이 내부적으로:
- 스피너 표시 (text 대신, `CircularProgressIndicator.adaptive`)
...
- **버튼 이중 탭 방지**: `PrimaryButton(isLoading: true)` 이 `onPressed: null` 로 비활성.
...
- [`lib/kits/auth_kit/ui/login/login_screen.dart`](https://github.com/storkspear/template-flutter/blob/main/lib/kits/auth_kit/ui/login/login_screen.dart) — `PrimaryButton(isLoading: ...)` 사용
