...
이 문서는 사용자별 알림 채널 toggle (push / email) 계약을 다뤄요. 짝 백엔드의 [`NotificationSettingController`](https://github.com/storkspear/template-spring/blob/main/core/core-billing-impl/src/main/java/com/factory/core/billing/impl/controller/NotificationSettingController.java) (ADR-031) 와 1:1 로 결합돼요.

Flutter 측 호출은 [`lib/kits/backend_api_kit/notification_settings.dart`](../../lib/kits/backend_api_kit/notification_settings.dart) 의 `NotificationSettings` 클래스가 담당해요.
...
> `/api/apps/{appSlug}` prefix 적용 대상이에요 — `ApiClient.get` / `ApiClient.patch` 의 자동 prefix 를 사용해요.
...
> 짝 백엔드의 `core-billing-api/NotificationKind.java` 와 **1:1 매핑** 이에요. 추가 시 양쪽 동시 변경이 필요해요. Flutter 측 enum 은 `kits/backend_api_kit/notification_settings.dart` 의 `NotificationKind` 예요.
...
- 미등록 kind 라도 **PATCH 1회로 등록**돼요 (server-side upsert).
...
1. `list()` 결과를 Map<NotificationKind, NotificationPreference> 로 인덱싱해요.
2. 각 kind 에 대해 응답에 있으면 그 값, 없으면 default `(push: true, email: true)` 를 사용해요.
3. toggle 변경 시 두 필드 모두 포함해 PATCH 해요.
