...
| Code | HTTP | Dart 상수 | 설명 |
|------|------|---------|------|
| `ATC_001` | 404 | `ErrorCode.fileNotFound` | 파일 미존재·비ACTIVE·소유/공개 불충족 **공용** (존재 은닉 — 전부 404) |
| `ATC_002` | 422 | `ErrorCode.contentTypeNotAllowed` | 업로드 content-type 화이트리스트 위반 (정확 매치) |
...
class ErrorCode {
  ...
  // ─── 첨부/파일 도메인 (Spring AttachmentError) ───────────────────────────
  static const fileNotFound = 'ATC_001'; // 404 (존재 은닉 공용)
  static const contentTypeNotAllowed = 'ATC_002'; // 422
  static const fileSizeExceeded = 'ATC_003'; // 413
  static const attachmentAssociationInvalid = 'ATC_004'; // 422
