...
> **ADR 과의 관계**: 컨벤션 문서는 **실행 규칙** (어떻게 쓸 것인가) 이고, ADR 은 **설계 결정의 근거** (왜 이렇게?) 예요. 각 컨벤션의 "왜?" 는 해당 ADR 에 링크되어 있어요.
...
| [`viewmodel-mvvm.md`](./viewmodel-mvvm.md) | `Notifier + ConsumerWidget` 패턴 | [`ADR-005`](../philosophy/adr-005-riverpod-mvvm.md) |
...
> **테스트 규약**은 별도 섹션이에요. [`Testing Strategy`](../testing/testing-strategy.md) · [`Contract Testing`](../testing/contract-testing.md) 를 참조하세요.

---

## 규약의 우선순위

1. **동작하는 코드** 가 이상적인 규약보다 우선이에요
2. **이 문서에 명시된 규약** 이 개인 취향보다 우선이에요
3. **프로젝트 내 기존 패턴** 이 새 패턴보다 우선이에요
4. **SOLID / DRY / YAGNI** 같은 원칙은 참조점이지 절대 기준이 아니에요
...
**별도 "docs 업데이트" 커밋으로 분리하지 않아요**. 코드 변경과 한 커밋으로 묶어요.

---

## 문서 작성 스타일 (docs/ 하위 모든 마크다운)

자세한 건 [`STYLE_GUIDE`](../STYLE_GUIDE.md) 를 참조하세요. 여기엔 핵심만 추려요.
...
- **독자 레벨 Level 2** (실무 중급 Flutter 개발자) 를 기본으로 써요

---

## 파생 레포가 이 규약을 확장할 때

파생 레포는 **템플릿 원본 규약을 그대로 유지** 하는 게 기본이에요. 도메인별 추가 규칙이 필요하면 파생 레포의 `docs/conventions/` 에 **새 파일** 로 추가하고, 템플릿 원본 파일은 건드리지 않아요. 이래야 cherry-pick 전파 시 충돌이 최소화돼요.
